 ...
 * 账号密码登录后抓这个链接响应体data中token得值V2_XXXXXXXXXX  和data中user_info中mid  两个用&链接
 * 例如 V2_XXXXXXXXXX&0jixxxxbml_mhy&1
 * 最后有个1哦记住,1是IOS  2是安卓 按需写入
 * 亦或者直接搜关键词stoken和mid 找到后取其值 填入变量中
...
let scriptVersionNow = "0.0.2"; //现在版本
//---------------------- 自定义变量区域 -----------------------------------
...
const device_id = `1ece0c2f-67ba-39fb-b1d1-8aaaaaaaaaaa`;
//---------------------------------------------------------
...
class UserInfo {
  ...
  getUUID() {
    return Array.from({ length: 32 }, () =>
      ...
      .map((value, index) =>
        [7, 11, 15, 19].includes(index) ? `-${value}` : value
      )
      .join("");
  }
    ...
    try {
      let options = {
          ...
          headers: {
            ...
            Cookie: `stoken=${this.stoken}; mid=${this.mid}; aliyungf_tc=;`,
          },
        },
  ...
  async get_info_ys() {
    try {
      let options = {
          ...
          headers: {
            DS: this.getDS(),
            cookie: `stuid=${this.uid}; stoken=${this.stoken};mid=${this.mid}; aliyungf_tc=;`,
            "x-rpc-client_type": "2",
            "x-rpc-app_version": app_version,

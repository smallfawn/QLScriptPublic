 ...
 * 卡夫味 兑换E卡
 *
 * cron 0 0,7 * * *  kfw_exchange.js         
 *  多账号并行执行任务模板
 */
//=====================================================//
const $ = new Env("卡夫味兑换E卡");
...
async function userinfo() {
    try {
        ...
        if (result?.error_code == 0) {
            //console.log(`账号[` + Number(i + 1) + `]` + `用户:${result.data.nickname} 🎉`);
...
async function exchange(nickname) {
    try {
        let url = {
            url: `${hostname}/crm/public/index.php/api/v1/exchangeIntegral`,
            headers: {
                "Host": host,
                "token": ck[0],
                "Content-Length": 84,
            ...
            body: "value=%E4%BA%AC%E4%B8%9CE%E5%8D%A12%E5%85%83&phone=&type=%E8%A7%86%E9%A2%91%E5%8D%A1"
        ...
        if (result?.error_code == 0) {
            ...
            console.log(`账号[` + Number(i + 1) + `]用户为:[` + nickname + `]兑换失败!,可能目前缺货`)
        } else {
            console.log(`账号[` + Number(i + 1) + `]用户为:[` + nickname + `]兑换失败!${result.msg},可能目前缺货`)
        }
    } catch (error) {
        //console.log(error);
        console.log("卡夫味服务器卡爆啦");

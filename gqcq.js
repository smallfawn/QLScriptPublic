...
class UserInfo {
    ...
    async add_comment(topic_id) {//userinfo
        try {
            let options = {
                ...
                headers: this.headersOne,
                body: `commentType=0&postId=${topic_id}&commentContent=${this.getCommentText()}&commentId=0&commentatorId=NDc3ODY1MA==&isReplyComment=1`
            }
            //console.log(options);
            let result = await httpRequest(options);
            //console.log(result);
    ...
    async delete_topic(topic_id) {//userinfo
        try {
            let options = {
